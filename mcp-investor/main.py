...
def execute_investments(distribution_dict: dict, sheet_name: str) -> None:
    """
    ...
    for idx, tick_xls in enumerate(ws["A"][1:], start=1):
        current_investment = ws[investment_letter][idx].value
        for tick_inv, quant in distribution_dict.items():
            if tick_xls.value == tick_inv:
